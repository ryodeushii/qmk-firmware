...
read -r -a keymap_list <<<"${KEYMAPS:-default via srgb ryodeushii}"
...
if [[ ${#targets[@]} -eq 0 ]]; then
  printf 'No matching build targets found for KEYBOARDS=%s LAYOUTS=%s KEYMAPS=%s\n' "${KEYBOARDS:-air75v2 halo75v2 halo96v2 gem80}" "${LAYOUTS:-ansi iso}" "${KEYMAPS:-default via srgb ryodeushii}" >&2
  exit 1
fi
...
printf "Checkout modules/signalrgb/qmk_version.h for the version token used in this build"
git checkout modules/signalrgb/qmk_version.h
